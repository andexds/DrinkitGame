...
В конце метода `Complete` найди блок `_quests.RecordSale(order.recipe.id);` и идущий следом `return new OrderResolution { ... };`. Замени всё это на:
...
Смысл: счётчик заказов растёт на каждой выдаче, а `OrderCompleted` стреляет уже с готовым `resolution` перед возвратом.
